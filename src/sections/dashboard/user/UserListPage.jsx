...
export default function UserPage() {
  ...
  const filters = useSetState({ fullname: '', role: [], status: 'all' });
  ...
  const canReset =
    !!filters.state.fullname || filters.state.role.length > 0 || filters.state.status !== 'all';

  const notFound = (!dataFiltered.length && canReset) || !dataFiltered.length;

  const handleDeleteRows = useCallback(() => {
    const deleteRows = data.filter((row) => !table.selected.includes(row.id));
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  let stabilizedThis =
    inputData && inputData.length ? inputData.map((el, index) => [el, index]) : [];
